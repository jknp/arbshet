...
path = "D:/Nextcloud/Science/PhD/Projects/201909-08 PCa risk SNPs/"
options(readr.num_columns = 0)

lines <- data.frame(read.csv(paste0(path, "Analyses/Dep/PCa_lines.csv"), 
                             sep  = ";", check.names = F, header = T, stringsAsFactors = T))

ach_pca <- data.frame(read_delim(paste0(path, "Analyses/Dep/200310_achilles_pca.txt"), 
    "\t", escape_double = FALSE, trim_ws = TRUE))

LN_ARDEGs <- read_delim(paste0(path, "Cell lines/LNCaP/LN_andrDEGv2.txt"), 
    "\t", escape_double = FALSE, trim_ws = TRUE)

essen <- read_delim(paste0(path, "Analyses/Dep/Achilles_common_essentials.txt"), 
    "\t", escape_double = FALSE, trim_ws = TRUE, col_names = T)

loops <- read_delim(paste0(path,"Analyses/Dep/GRanges_loops.txt"), 
     "\t", escape_double = FALSE, trim_ws = TRUE, col_names = T)
 
netw_gns <- read_delim(paste0(path,"Analyses/Dep/netw_genes.txt"), 
    "\t", escape_double = FALSE, trim_ws = TRUE, col_names = T)
...
### Example depency plot for VCAP with fixed jitter seed
```{r VCAP plot}
...
### plots with correct jittered labels - not yet right
VCdep <- ggplot(VC_bin_deps, aes(x=val, id, color = col)) +  #color = ifelse(val < -.5, ARtal, 0)
  #scale_color_gradient(low = "dodgerblue3", high = "red1", limits = c(1,max(VC_bin_deps$ARtal))) +
  scale_colour_identity() +   
  geom_point(position = position_jitter(seed = 1L), aes(color = col)) +
  geom_text(data = . %>% filter(val < -.6), size = 2, check_overlap = F,
              aes(val, id, label = gene)) +
  ...
  labs(color = "ARenh tally") + coord_cartesian(xlim = c(-2, 0.5)) #+
  #ggtitle("Achilles gene dependencies of PCa cell +R1881 AR DEGs looping to tally enhancers(FDR < .05)")
...
### plots with correct jittered labels - not yet right
RVdep <- ggplot(RV_bin_deps, aes(x=val, id, color = col)) +  #color = ifelse(val < -.5, ARtal, 0)
  #scale_color_gradient(low = "dodgerblue3", high = "red1", limits = c(1,max(RV_bin_deps$ARtal))) +
  scale_colour_identity() +   
  geom_point(position = position_jitter(seed = 1L), aes(color = col)) +
  geom_text(data = . %>% filter(val < -.6), size = 2, check_overlap = F,
              aes(val, id, label = gene)) +
...
### plots with correct jittered labels - not yet right
LNdep <- ggplot(LN_bin_deps, aes(x=val, id, color = col)) +  #color = ifelse(val < -.5, ARtal, 0)
  #scale_color_gradient(low = "dodgerblue3", high = "red1", limits = c(1,max(LN_bin_deps$ARtal))) +
  scale_colour_identity() +   
  geom_point(position = position_jitter(seed = 1L), aes(color = col)) +
  geom_text(data = . %>% filter(val < -.6), size = 2, check_overlap = F,
              aes(val, id, label = gene)) +
